...
export async function createMetaMaskSmartAccount(chainId: number) {
  ...
  // Tạo salt unique per EOA để mỗi ví có 1 Smart Account cố định
...
/**
 * Check if Smart Account đã tồn tại cho EOA này
 * @param chainId - Chain ID (11155111 for Sepolia, 10143 for Monad)
 * @returns { address: string, exists: boolean }
 */
export async function checkSmartAccountExists(chainId: number) {
  if (typeof window === 'undefined') {
    throw new Error("This function can only be called on the client side");
  }
  ...
  // Tạo salt unique per EOA
  const salt = keccak256(toHex(address));
  
  // Tạo Smart Account để lấy address
  const smartAccount = await toMetaMaskSmartAccount({
    client: publicClient,
    implementation: Implementation.Hybrid,
    deployParams: [address, [], [], []],
    ...
    signer: { walletClient: null as any }, // Chỉ cần để tính address
  });
